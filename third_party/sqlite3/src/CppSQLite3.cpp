...
namespace detail {
  ...
  SQLite3Memory::SQLite3Memory() : mnBufferLen(0), mpBuf(nullptr)
  {
  }

  SQLite3Memory::SQLite3Memory(int nBufferLen) : mnBufferLen(nBufferLen), mpBuf(sqlite3_malloc(nBufferLen))
  {
    if (!mpBuf && mnBufferLen > 0) {
      throw CppSQLite3Exception(CPPSQLITE_ERROR, ALLOCATION_ERROR_MESSAGE, DONT_DELETE_MSG);
    }
  }

  SQLite3Memory::SQLite3Memory(const char *szFormat, va_list list) : mnBufferLen(0), mpBuf(sqlite3_vmprintf(szFormat, list))
  {
    if (!mpBuf) {
      throw CppSQLite3Exception(CPPSQLITE_ERROR, ALLOCATION_ERROR_MESSAGE, DONT_DELETE_MSG);
    }
  ...
  SQLite3Memory::SQLite3Memory(SQLite3Memory const &other) : mnBufferLen(other.mnBufferLen), mpBuf(sqlite3_malloc(other.mnBufferLen))
  {
    if (!mpBuf && mnBufferLen > 0) {
      throw CppSQLite3Exception(CPPSQLITE_ERROR, ALLOCATION_ERROR_MESSAGE, DONT_DELETE_MSG);
    }
  ...
  SQLite3Memory::SQLite3Memory(SQLite3Memory &&other) : mnBufferLen(other.mnBufferLen), mpBuf(other.mpBuf)
  {
    other.mnBufferLen = 0;
    other.mpBuf       = nullptr;
  }
...
CppSQLite3Exception::CppSQLite3Exception(const int nErrCode, const char *szErrMess, bool bDeleteMsg /*=true*/) : mnErrCode(nErrCode)
{
...
size_t CppSQLite3Query::getSizeTField(int nField, long long nNullValue /*=0*/) const
{
  if (fieldDataType(nField) == SQLITE_NULL) {
    return nNullValue;
  } else {
...
size_t CppSQLite3Query::getSizeTField(const char *szField, long long nNullValue /*=0*/) const
{
  int nField = fieldIndex(szField);
  return getInt64Field(nField, nNullValue);
}
...
void CppSQLite3Statement::bindInt64(int nParam, const long long nValue)
{
  checkVM();
  int nRes = sqlite3_bind_int64(mpVM, nParam, nValue);
...
void CppSQLite3Statement::bindInt64(const char *szParam, const long long nValue)
{
  long long nParam = bindParameterIndex(szParam);
  bindInt64(nParam, nValue);
}

void CppSQLite3Statement::bindSizeT(const char *szParam, size_t nValue)
{
  long long nParam = bindParameterIndex(szParam);
  bindInt64(nParam, nValue);
}
...
bool CppSQLite3DB::tableExists(const char *szTable)
{
  ...
  sql.format("select count(*) from sqlite_master where type='table' and name=%Q", szTable);
